...
async function loadCurrentEmployee() {
    ...
    try {
        ...
        await loadAllShiftsForApproval(); // Cargar todos los shifts pendientes
        await initChat();
...
// ==================== SHIFT APPROVAL - NUEVA SECCIÓN ====================
async function loadAllShiftsForApproval() {
    const container = $('shiftApprovalList');
    if (!container) return;
    
    container.innerHTML = '<div class="loading">Loading shift requests...</div>';
    
    try {
        // Obtener todos los empleados con shifts pendientes
        const employeesSnap = await getDocs(collection(db, "employeeRecords"));
        const pendingShifts = [];
        ...
        if (pendingShifts.length === 0) {
            container.innerHTML = `
                <div class="empty-state">
                    ...
                    <div class="empty-desc">All shift requests have been processed.</div>
                </div>
            `;
            return;
        }
            ...
            card.innerHTML = `
                ...
                <div class="shift-approval-details">
                    ...
                    <div class="detail-row">
                        ...
                        <span class="detail-value">${shift.selectedAt ? new Date(shift.selectedAt.toDate()).toLocaleDateString() : 'Unknown'}</span>
                    </div>
                </div>
                <div class="shift-approval-actions">
                    <button class="btn btn-success" onclick="window.confirmShift('${shift.empId}')">
                        Confirmar
                    </button>
                    <button class="btn btn-danger" onclick="window.rejectShift('${shift.empId}')">
                        Rechazar
                    </button>
                </div>
            `;
...
async function confirmShift(empId) {
    try {
        ...
        // Actualizar también en users collection
        ...
        loadAllShiftsForApproval(); // Recargar la lista
...
async function rejectShift(empId) {
    if (!confirm(`Are you sure you want to reject the shift for ${empId}?`)) return;
...
function initTabs() {
    document.querySelectorAll('.tab').forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            if (tabId === 'shift') loadAllShiftsForApproval();
...
export function initAdminApp() {
    ...
    loadAllEmployees();
    loadAllShiftsForApproval(); // Cargar shifts al inicio
