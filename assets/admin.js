...
import {
    ...
    serverTimestamp, orderBy, addDoc, arrayUnion
...
async function loadDashboard() {
    ...
    try {
        ...
        allowedSnap.forEach(doc => {
            ...
            // Check if onboarding is pending (no shift selected)
        ...
        uiToast('Error loading dashboard data');
    }
}
...
async function loadEmployeeList() {
    ...
    container.innerHTML = '<div style="color: #6b7280; padding: 20px;">Loading...</div>';
    
    if (!isFirebaseConfigured()) {
        container.innerHTML = '<div style="color: #6b7280; padding: 20px;">Preview mode - Firebase not connected</div>';
        return;
    }
    ...
    try {
        ...
        if (employees.length === 0) {
            container.innerHTML = '<div style="color: #6b7280; padding: 20px; text-align: center;">No employees registered yet</div>';
            return;
        }
        ...
        container.innerHTML = employees.map(emp => `
            <div class="employee-item">
                <div class="employee-info">
                    ...
                    <div class="employee-name">${escapeHtml(emp.name || 'No name set')}</div>
                </div>
                <div style="display: flex; gap: 10px; align-items: center;">
                    <span class="status-badge ${emp.active !== false ? 'status-active' : 'status-inactive'}">
                        ${emp.active !== false ? 'Active' : 'Inactive'}
                    </span>
                    <button class="btn-danger" data-empid="${emp.id}" data-active="${emp.active !== false}">
        ...
        container.querySelectorAll('.btn-danger').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                toggleEmployeeStatus(empId, currentActive);
            });
        });
...
async function addEmployee() {
    ...
    if (!empId) {
        uiToast('Invalid Employee ID format. Use SP### format (e.g., SP001)');
        return;
    }
...
async function loadProfile() {
    const searchInput = document.getElementById('searchProfileId');
    if (!searchInput) return;
    
    const empId = normalizeEmpId(searchInput.value);
    
    if (!empId) {
        uiToast('Invalid Employee ID format');
        return;
    }
    ...
    try {
        ...
        document.getElementById('profileEditor')?.classList.remove('hidden');
        uiToast(`Profile loaded for ${empId}`);
...
async function saveProfile() {
    ...
    try {
        ...
        // Update name in allowedEmployees
        const empRef = doc(db, "allowedEmployees", currentProfileEmpId);
...
async function loadAppointment() {
    const searchInput = document.getElementById('apptEmpId');
    if (!searchInput) return;
    ...
    try {
        ...
        document.getElementById('apptEditor')?.classList.remove('hidden');
        uiToast(`Appointment loaded for ${empId}`);
...
async function sendNotification() {
    ...
    const actionInput = document.getElementById('notifAction');
    const routeInput = document.getElementById('notifRoute');
    
    const target = targetInput?.value.trim().toUpperCase() || '';
    const type = typeInput?.value || 'company';
    const title = titleInput?.value.trim() || '';
    const body = bodyInput?.value.trim() || '';
    const action = actionInput?.value.trim() || 'View Details';
    const route = routeInput?.value.trim() || 'home';
    ...
    try {
        if (target === 'ALL') {
            ...
            uiToast(`Notification sent to ${count} employees`);
...
async function loadNotificationHistory() {
    ...
    if (!isFirebaseConfigured()) {
        container.innerHTML = '<div style="color: #6b7280; padding: 20px;">Preview mode</div>';
        return;
    }
    
    try {
        container.innerHTML = '<div style="color: #6b7280; padding: 20px;">Loading history...</div>';
        // In a real app, you'd query a notifications collection
        // For now, show placeholder
        setTimeout(() => {
            container.innerHTML = '<div style="color: #6b7280; padding: 20px; text-align: center;">Notification history will appear here</div>';
        }, 500);
    } catch (e) {
        container.innerHTML = `<div style="color: #dc2626; padding: 20px;">Error loading history</div>`;
    }
}
...
async function loadChatMessages(empId) {
    ...
    messagesDiv.innerHTML = '<div style="text-align: center; color: #9ca3af; padding: 40px;">Loading messages...</div>';
    
    if (!isFirebaseConfigured()) {
        messagesDiv.innerHTML = '<div style="text-align: center; color: #9ca3af; padding: 40px;">Preview mode - Chat not available</div>';
        return;
    }
    ...
    try {
        ...
        messagesDiv.innerHTML = messages.map(msg => `
            <div class="message ${msg.sender === 'admin' ? 'message-admin' : 'message-employee'}">
                <div style="font-size: 12px; opacity: 0.8; margin-bottom: 4px;">
                    ${msg.sender === 'admin' ? 'You (Admin)' : 'Employee'} • ${new Date(msg.timestamp?.toDate?.() || msg.timestamp).toLocaleString()}
                </div>
        ...
        messagesDiv.innerHTML = `<div style="color: #dc2626; padding: 20px;">Error loading messages</div>`;
    }
}
...
async function sendChatMessage() {
    ...
    if (!text || !selectedChatEmployee) {
        uiToast('Select an employee and type a message');
        return;
    }
...
async function saveCompanySettings() {
    const settings = {
        hrEmail: document.getElementById('settingHrEmail')?.value.trim() || 'hr@sunpowerc.energy',
        hrPhone: document.getElementById('settingHrPhone')?.value.trim() || '800-876-4321',
        managerPhone: document.getElementById('settingManagerPhone')?.value.trim() || '502-467-8976',
        supervisorPhone: document.getElementById('settingSupervisorPhone')?.value.trim() || '615-786-9543',
        safetyPhone: document.getElementById('settingSafetyPhone')?.value.trim() || '502-555-0172',
        shopUrl: document.getElementById('settingShopUrl')?.value.trim() || 'https://shop.sunpowerc.energy'
    };
    ...
    try {
        ...
        uiToast('Company settings saved successfully');
    } catch (e) {
        uiToast(`Error: ${e.message}`);
    }
}
