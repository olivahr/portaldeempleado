...
function renderShiftSelection(userData, saveUserPatch) {
  ...
  // Si ya está aprobado, mostrar mensaje de aprobado
  if (status.isDone && shiftApproved) {
    setPage(
      "Shift Selection",
      "Approved",
      `
  ...
  // Si ya seleccionó pero está pendiente de aprobación
  if (status.isDone && !shiftApproved) {
    setPage(
      "Shift Selection",
      "Pending Approval",
      `
        <div class="azCard" style="text-align:center;padding:40px 24px;background:linear-gradient(135deg,rgba(245,158,11,.08),rgba(245,158,11,.02));border-color:rgba(245,158,11,.25);">
          ...
          <div style="font-weight:1000;font-size:20px;color:rgba(2,6,23,.85);margin-bottom:8px;">Pending por Aprobación</div>
          <div style="font-size:14px;color:rgba(2,6,23,.60);line-height:1.5;margin-bottom:24px;">
            Tu selección de shift está pendiente de aprobación por HR.<br>
          ...
          <div class="azCard" style="text-align:left;margin-bottom:24px;background:#fff;">
            ...
            <div style="margin-top:12px;padding:8px 12px;background:rgba(245,158,11,.10);border-radius:8px;">
              <div style="font-size:12px;color:rgba(180,83,9,1);font-weight:1000;">⏳ Status: Pending Approval</div>
            </div>
          </div>
          <div class="muted" style="font-size:13px;">
            Por favor espera la confirmación de HR antes de continuar.
          </div>
        </div>
      `
    );
  ...
  document.getElementById("btnShiftSave").onclick = async () => {
    ...
    triggerConfetti();
    uiToast("Preferences saved! Pending HR approval.");
    location.hash = "#shift";
  };
}
...
function renderFootwear(userData, saveUserPatch, publicData) {
  ...
  setPage(
    ...
    `
      ...
      <!-- Botón para continuar al I-9 después de verificar tienda -->
      <div id="continueToI9Section" style="display:none;margin-top:16px;">
        <div class="azCard" style="background:linear-gradient(135deg,rgba(29,78,216,.08),rgba(22,163,74,.04));border-color:rgba(29,78,216,.30);">
          <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
            <div style="width:40px;height:40px;border-radius:999px;background:rgba(29,78,216,.10);display:flex;align-items:center;justify-content:center;color:rgba(29,78,216,1);">
              ${azIcon("checkCircle")}
            </div>
            <div>
              <div class="azCardTitle">Ready to Continue</div>
              <div class="azCardSub">You've reviewed the safety footwear requirements</div>
            </div>
          </div>
          <a class="btn primary" href="#i9" style="display:block;width:100%;text-align:center;border-radius:16px;padding:16px;">
            Continuar al I-9
          </a>
        </div>
      </div>
  ...
  const syncBtn = () => {
    ...
    // Mostrar botón de continuar al I-9 si todos los acks están listos
    const continueSection = document.getElementById("continueToI9Section");
    if (continueSection) {
      continueSection.style.display = allAcks ? "block" : "none";
    }
  };
