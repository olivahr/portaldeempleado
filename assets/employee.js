...
// ✅ AFTER setPage(...) — wire events + save + success/confetti
ensureShiftSuccessUI();

const btnSave = document.getElementById("btnShiftSave");

// helpers: lee radios aunque cambien nombres
const getChecked = (names) => {
...
// si tus radios tienen otros "name", ponlos aquí
const getPosPick = () => getChecked(["posPick", "positionPick", "position", "pos"]);
const getShiftPick = () => getChecked(["shiftPick", "shift", "shiftChoice", "sh"]);

if (btnSave) {
  btnSave.onclick = async () => {
    const posPick = getPosPick();
    const shPick  = getShiftPick();

    if (!posPick || !shPick) {
      if (typeof toast === "function") toast("Select a position and a shift to continue.");
      else alert("Select a position and a shift to continue.");
      return;
    }
    ...
    await saveUserPatch({
      ...
      // ✅ OPTIONAL: si tú manejas progress en el doc (si no lo usas, quítalo)
      progress: {
        shift_selection: true,
        shiftSelectionAt: Date.now()
      }
    });

    // labels bonitos para el modal
    const posTitle =
      posPick === "assembler" ? "Solar Panel Assembler" :
      posPick === "material"  ? "Material Handler / Warehouse" :
      posPick === "qc"        ? "Quality Control / Inspection" :
      "Selected Position";

    const shTitle =
      shPick === "early" ? "Early Shift (6:00 AM – 2:30 PM)" :
      shPick === "mid"   ? "Mid Shift (2:00 PM – 10:30 PM)" :
      shPick === "late"  ? "Late Shift (10:00 PM – 6:30 AM)" :
      "Selected Shift";

    // ✅ confetti + modal
    openShiftSuccess({ posTitle, shTitle });

    // ✅ si quieres mandarlo al próximo paso automático (cámbialo si tu ruta es otra)
    // setTimeout(() => { location.hash = "#footwearpolicy"; }, 900);
  };
}

/* ---------- Success UI + Confetti (no CSS here) ---------- */

function ensureShiftSuccessUI() {
  if (document.getElementById("ssSuccessBackdrop")) return;

  const wrap = document.createElement("div");
  wrap.innerHTML = `
    <div id="ssSuccessBackdrop" class="ss-success-backdrop" style="display:none;">
      <div class="ss-success-card" role="dialog" aria-modal="true" aria-labelledby="ssSuccessTitle">
        <div class="ss-success-top">
          <div class="ss-success-icon" aria-hidden="true">✓</div>
          <div>
            <div id="ssSuccessTitle" class="ss-success-title">Preferences Saved</div>
            <div class="ss-success-sub" id="ssSuccessSub">Your shift preference has been recorded.</div>
          </div>
        </div>

        <div class="ss-success-summary" id="ssSuccessSummary"></div>

        <div class="ss-success-actions">
          <button type="button" class="btn ghost" id="ssSuccessClose">Close</button>
          <button type="button" class="btn primary" id="ssSuccessContinue">Continue</button>
        </div>

        <canvas id="ssConfetti" class="ss-confetti" aria-hidden="true"></canvas>
      </div>
    </div>
  `;
  document.body.appendChild(wrap);

  document.getElementById("ssSuccessClose").onclick = closeShiftSuccess;
  document.getElementById("ssSuccessBackdrop").addEventListener("click", (e) => {
    if (e.target?.id === "ssSuccessBackdrop") closeShiftSuccess();
  });

  document.getElementById("ssSuccessContinue").onclick = () => {
    closeShiftSuccess();
    // ✅ manda al próximo paso si quieres (cambia la ruta)
    // location.hash = "#footwearpolicy";
  };
}

function openShiftSuccess({ posTitle, shTitle }) {
  const bd = document.getElementById("ssSuccessBackdrop");
  const sub = document.getElementById("ssSuccessSub");
  const sum = document.getElementById("ssSuccessSummary");

  sub.textContent = "Congratulations — your shift preference has been submitted for HR confirmation.";
  sum.innerHTML = `
    <div class="ss-sum-row"><span class="muted">Position</span><strong>${escapeHtml(posTitle)}</strong></div>
    <div class="ss-sum-row"><span class="muted">Shift</span><strong>${escapeHtml(shTitle)}</strong></div>
    <div class="ss-sum-note">You can update this later if HR requests changes.</div>
  `;
...
function launchConfetti() {
  const canvas = document.getElementById("ssConfetti");
  if (!canvas) return;

  const card = canvas.closest(".ss-success-card");
