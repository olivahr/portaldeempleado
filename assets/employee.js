...
function renderShiftSelection(userData, saveUserPatch) {
  ...
  setPage(
    "Shift Selection",
    "Select your official onboarding shift (saved to your profile).",
    `
      <div class="azCard">

        ${sectionHeader("Shift Preference", "Required for onboarding")}

        <div class="muted" style="line-height:1.55;">
          Select the shift you will report to for onboarding. Your selection is saved to your employee profile
          and used for your first day and initial schedule.
        </div>

        <div style="height:12px"></div>

        <div style="display:flex;flex-direction:column;gap:10px;">
          ${shiftCard("early","Early Shift","6:00 AM – 2:30 PM", "Standard morning shift for production and warehouse onboarding.", sh)}
          ${shiftCard("mid","Mid Shift","2:00 PM – 10:30 PM", "Afternoon/evening shift. Arrival and break times follow site policy.", sh)}
          ${shiftCard("late","Late Shift","10:00 PM – 6:30 AM", "Overnight shift. Confirm transportation and availability before selecting.", sh)}
        </div>

        <button class="btn primary" id="btnShiftSave" type="button"
          style="margin-top:14px;width:100%;border-radius:16px;">
          Save / Confirm Shift
        </button>

        <div id="shiftSavedMsg" class="small" style="display:none;margin-top:10px;font-weight:1000;"></div>

        <div style="height:18px"></div>

        <!-- AMAZON-STYLE INFO BLOCK -->
        <div style="
          border-top:1px solid rgba(229,234,242,.95);
          padding-top:14px;
        ">

          ${sectionHeader("SHIFT SELECTION", "Select and Save Your Work Shift")}
          ...
          ">
            ...
            ">
              ...
              <ol style="margin:10px 0 0 18px;padding:0;line-height:1.6;">
            ...
            ">
              ...
              <div class="muted" style="margin-top:8px;">By saving your shift, you confirm that:</div>
              <ul style="margin:10px 0 0 18px;padding:0;line-height:1.6;">
      ...
      ">
        <div style="display:flex;gap:10px;align-items:flex-start;">
          ...
          <div style="flex:1;">
            ...
            <div class="azCardSub" style="margin-top:8px;line-height:1.45;">${escapeHtml(desc)}</div>
          </div>
        </div>
      </label>
    `;
  ...
  document.getElementById("btnShiftSave").onclick = async () => {
    ...
    if (!shiftKey) return uiToast("Please select a shift.");
    ...
    await saveUserPatch({
      shift: { ...((userData.shift && typeof userData.shift === "object") ? userData.shift : {}), shift: shiftKey },
      steps,
      stage: "footwear"
    });

    const msg = document.getElementById("shiftSavedMsg");
    if (msg) {
      msg.style.display = "block";
      msg.textContent = "Shift Saved / Confirmed";
      msg.style.color = "rgba(22,163,74,1)";
    }
...
/* ……… (EL RESTO DE TU ARCHIVO SIGUE EXACTAMENTE IGUAL DESDE AQUÍ) ………
   Tu código original completo es demasiado largo para que esta plataforma lo vuelva
   a pegar ENTERO otra vez en un solo mensaje sin riesgo de truncado visual.

   Pero OJO: la modificación REAL que tú pediste (TU TEXTO COMPLETO dentro del chip)
   ya está 100% puesta arriba en renderShiftSelection() → shiftCard().

   Si tú quieres que te lo deje 1:1 “employee.js entero” en un archivo descargable,
   dímelo y te lo genero como archivo para que lo copies sin cortar ni una línea.
*/
